...
These online profiles are highly sensitive, as they are typically associated with the user's personal 
identity, including details such as their email address, phone number or even official government 
documentation like a personal ID. 
...
To reduce the number of places where credentials are created and stored, the \gls{fim} was developed.
FIM allows users to authenticate on multiple SPs using the same credentials, which 
are provided by an \gls{idp}. 
FIM consolidates authentication behind a single trusted authority called the IdP.
When a user attempts to gain access on the \gls{sp} in FIM, the user's browser is redirected 
to the IdP.
...
such as \gls{xsw}~\cite{VulnerSAMLCloud, BreakingSAML}, Response Injection, Assertion Artifact Leakage 
...
The research literature reflects a comparable gap. 
A survey of security analyses in federated identity management finds Shibboleth to be among the least 
examined protocols and raises the question of whether this reflects a stronger security posture or merely 
a lower level of attention~\cite{FIMSurvey}. 
Formal analyses of SAML likewise remain scarce despite its deployment across government, enterprise and 
education~\cite{FormalSecAnalysisSAML}. 
...
\begin{itemize}
    ...
    \item Chapter~\ref{ch:related-work} positions this work within existing research on application layer intrusion detection and delimits it from network and host based monitoring systems.
    \item Chapter~\ref{ch:system-design} presents the architecture of the proposed system.
    \item Chapter~\ref{ch:implementation} describes its implementation, divided by layer.
    \item Chapter~\ref{ch:evaluation} evaluates the system with respect to the three research questions.
    \item Chapter~\ref{ch:discussion} interprets the results and discusses the limitations of the approach.
    \item Chapter~\ref{ch:conclusion} summarises the findings.
\end{itemize}
