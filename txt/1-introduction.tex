...
to authenticate at \acrshort{sp_plu} as the affected user.
...
\acrshort{fim} allows users to authenticate on multiple \acrshort{sp_plu} using the same credentials, which 
are provided by an \acrfull{idp}. 
\acrshort{fim} consolidates authentication behind a single trusted authority called 
the \acrshort{idp}.
When a user attempts to gain access on the \acrfull{sp} in \acrshort{fim}, the user's browser is redirected 
to the \acrshort{idp}.
The user authenticates with the \acrshort{idp} and the \acrshort{idp} communicates with the \acrshort{sp} 
to guarantee that the user is who they say they are.
...
Under this model, a user authenticates with their organisation's \acrshort{idp} once.
The \acrshort{idp} then issues cryptographically signed SAML assertions that \acrshort{sp_plu} accept 
as proof of identity.
This architecture is widely deployed in higher education and research federation such as DFN-AAI~\cite{DFNAAI}, 
where a single \acrshort{idp} serves dozens of \acrshort{sp_plu} simultaneously.

This architectural property introduces a critical consequence: the \acrshort{idp} becomes a single point of trust 
for the entire federation. 
A compromised, misconfigured or vulnerable \acrshort{idp} does not affect one application.
It undermines the security of every relying \acrshort{sp}.
...
These vulnerabilities allow an attacker to forge or manipulate the security claim that the security claims 
that the \acrshort{sp} accepts as authoritative once the signature validates.

Despite this elevated risk profile, systematic security monitoring at the application layer of Shibboleth 
\acrshort{idp} deployments remains largely absent. 
...
scarce despite its deployment across government, enterprise and education~\cite{FIMSurvey, FormalSecAnalysisSAML}.
...
of a network node, the proposed system acts as a Security Looking Glass for the Shibboleth \acrshort{idp}. 
...
\begin{itemize}
    \item RQ1 Which security-relevant data can be collected at the application layer of a Shibboleth \acrshort{idp}, and through which mechanisms are these data accessible?
    ...
    \item RQ3 What measurable overhead does the resulting system impose on the monitored \acrshort{idp}?
\end{itemize}

The remainder of this thesis is structured as follows. 
\begin{itemize}
    \item Chapter \ref{ch:background} introduces the technical foundations of \acrshort{fim}, SAML and the Shibboleth \acrshort{idp}. 
