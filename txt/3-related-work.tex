...
\section{Security Analyses of SAML and Shibboleth}\label{sec:saml-analyses}

The security of SAML-based federations has been examined from two directions. 
Empirical studies analyse existing implementations, while formal approaches reason about the 
specification itself.

Somorovsky et al. examined fourteen SAML frameworks and found critical XSW vulnerabilities in 
eleven of them~\cite{BreakingSAML}. 
Their analysis is confined to the processing of assertions at the SP, and it does not consider 
what information is available at the IdP during or after such an attack. 
Mainka et al. extended this line of work to cloud environments and reported that a majority of 
the analysed SaaS providers were vulnerable to at least one of eight identified attack 
vectors~\cite{VulnerSAMLCloud}.

A second line of work approaches the problem formally. 
Sommer analyses the Shibboleth implementation with respect to authentication, session integrity 
and logout consistency~\cite{SecAnalysisSSO}, while Hartl and Derek analyse the SAML 
specification itself~\cite{FormalSecAnalysisSAML}. 
The latter derive eight protocol variants from the optional features of a single use case and 
prove the security properties of each separately. 
Their premise is that SAML is not a security protocol in itself, but a framework whose optional 
features can be composed in different ways.

Both approaches establish which configurations are secure under a given set of assumptions. 
Neither verifies whether a deployed system actually satisfies those assumptions. 
Formal analysis therefore answers which configuration is secure, whereas this work observes 
which configuration is in effect.

\section{Why Monitor from the Inside?}\label{sec:why-inside}

As discussed in the Chapter~\ref{ch:introduction}, the proposed monitoring system operates 
...
A network firewall primarily controls network communication, while a network-based \gls{ids} 
has limited visibility into application-specific behaviour. 
A \gls{waf} can provide additional protection against generic application-layer attacks, such 
...
Network-based IDS, such as Suricata and Snort, operate primarily at the network level, 
whereas \gls{edr} solutions operate at the host level. 
...
A complementary perspective is provided by Martín and Beltrán, who propose 
behavioural anomaly detection at the SP rather than the IdP~\cite{UEBAFederation}. 
Their argument is that delegating authentication to an IdP does not transfer the entire security 
responsibility, and that the relying party can contribute to detection even after authentication 
has succeeded. 
The IdP side is explicitly outside their scope, and protocol-level information such as assertion 
structure, attribute release policies or SP metadata is consequently not considered. 
Taken together with the present work, the two perspectives address opposite ends of the 
same federation.

\section{Why SIEM Functionality Is Not Enough}\label{sec:why-not-siem}

A \gls{siem} primarily operates on logs and security-relevant events. 
...
IE6, \textit{Violation of Security Log Integrity}, addresses the integrity of 
security logs and cites NIST SP 800-92 and Snodgrass et al. as supporting 
references~\cite{OWASPAppSensor, NIST80092, TamperDetection}. 
