...
The monitoring tool provides separate endpoints for different use cases. 
Each aggregation component has a dedicated endpoint for transmitting its data, thereby separating 
the individual responsibilities of the components within the system. 

As mentioned in Section~\ref{sec:trust}, the interceptor sends the payload asynchronously. 
This means that the interceptor does not wait for a response from the monitoring tool and can 
immediately proceed with the next authentication process.
However, this also means that, in the event of packet loss, the interceptor receives no information 
about whether the payload was successfully delivered.
To measure the overhead introduced by the interceptor, an option to enable synchronous payload 
delivery was implemented. 
In synchronous mode, the interceptor sends the payload to the backend application and waits for 
a response before proceeding with the next authentication process.
The exact overhead introduced by the interceptor is discussed in Section~\ref{sec:overhead}.
