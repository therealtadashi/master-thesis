...
\subsubsection{Influence of \textit{ipHash}}\label{subsubsec:influence-of-iphash}

It remained unclear whether \textit{ipHash} actually contributed to the separation in the multi-user 
datasets, or whether \textit{usernameHash} alone accounted for the measured precision the assumption 
was that there was a shared \textit{ipHash} across all benchmark processes. 
A control query on the raw data refuted this for the browser path. 
Each \texttt{BrowserWebDriverContainer} started in parallel is assigned its own Docker Bridge IP. 
In Section~\ref{subsubsec:precision-multiple-browser-users}, the three test users thus actually had 
three different \textit{ipHash} values, not a shared one. 

This could be verified directly against the existing ground truth without a new login run. 
\texttt{CorrelationPrecisionAnalysis} was supplemented with a third, purely analytical strategy that 
searches only for SP + \textit{ipHash} + \textit{time window} (\textit{usernameHash} omitted). 
Not a production strategy, but a diagnostic variant to isolate the contribution of \textit{ipHash}. 
The data source is the same 606 reports from Section~\ref{subsubsec:precision-multiple-browser-users}, 
reanalyzed using the additional IP_ONLY strategy (hypothetical). 

\begin{table}[h]
\centering
\caption{Correlation with \textit{ipHash} (n = 606)}\label{tab:ip-hash-correlation}
\small
\begin{tabular}{lrr}
\toprule
Strategy & Precision (all windows) & Candidates \\
\midrule
...
The comparison across all six window sizes shows no convergence, but rather exact agreement 
down to the second decimal place. 

\begin{table}[h]
\centering
\caption{Candidates with \textit{ipHash} (n = 606)}\label{tab:ip-hash-candidates}
\small
\begin{tabular}{lrr}
\toprule
Window & \texttt{HASH\_AND\_TIME\_WINDOW} Candidates & \texttt{IP\_ONLY} Candidates \\
\midrule
$\pm$1\,s  & 1,00  & 1,00  \\
$\pm$2\,s  & 2,19  & 2,19  \\
$\pm$5\,s  & 3,86  & 3,86  \\
...
Even when considering local login density, both strategies yield identical results, including 
in the otherwise critical <100ms bucket (138/138 matches, 100\%). 
In this dataset, \textit{usernameHash} never once filters out a candidate that \textit{ipHash} 
would not have already ruled out on its own. 
Because each browser container had its own stable IP address, every \textit{ipHash} bucket was 
already valid based on the IP alone.

This shows that \textit{ipHash} distinguishes between separate sources (different devices, networks, 
or, as here, containers) just as effectively as the full \texttt{HASH\_AND\_TIME\_WINDOW} combination. 
It does not show what happens when multiple users are behind the same IP, such as corporate NAT or 
shared Wi-Fi. 
Because this scenario could not be replicated with the existing test setup (one container per user). 

\subsubsection{Precision among Multiple Users Sharing the same IP Address (NAT Scenario)}\label{subsubsec:precision-nat}

Section~\ref{subsubsec:influence-of-iphash} proved the influence of \textit{ipHash} for different 
sources. 
The opposite is the most relevant case in praxis, which is multiple users behind the same IP 
address. 
This could be the same company NAT or public WLAN, which could not be reproduced in the test 
environment. 
For that a new tool was created, which manages multiple users as parallel threads inside a process 
instead of separate containers. 
Because all threads send from the same machine, they share the same source IP. 
A cardinality aggregation on \textit{ip\_hash.keyword} over the duration of the run confirms exactly 
one distinct value across all 600 documents, thus, the shared IP is not merely assumed but measured. 
The data source are three different test users each with 200 logins, that are started from the same 
process. 

\begin{table}[h]
\centering
\caption{Procision of Strategies in NAT-Scenario}\label{tab:precision-strategy-nat}
\small
\begin{tabular}{lrr}
\toprule
Strategy & Precision (All Windows 1-60s) & Coverage \\
\midrule
\texttt{HASH\_AND\_TIME\_WINDOW} & 100,00\% & 100\% \\
...
\begin{table}[h]
\centering
\caption{Procision of Strategies in NAT-Scenario after Local Login Density (Window = 10\,s)}\label{tab:precision-strategy-nat-local-density}
\small
\begin{tabular}{lrrr}
\toprule
Density Bucket & n & \texttt{HASH\_AND\_TIME\_WINDOW} & \texttt{TIME\_WINDOW} \\
...
\texttt{HASH\_AND\_TIME\_WINDOW} remains at 100\% despite the split IP. 
\textit{usernameHash} accounts for the entire separation on its own, regardless of whether 
\textit{ipHash} contributes anything. 
\texttt{IP\_ONLY} was used as a cross-check. 
The diagnostic strategy from Section~\ref{subsubsec:influence-of-iphash} yields exactly the same 
numbers here as \texttt{TIME\_WINDOW}, identical for every window size and every density bucket 
(73.67\% precision, 442 hits, 158 misclassifications). 
This is no coincidence, but rather the inevitable consequence of a single shared IP. 
A filter for exactly this one \textit{ipHash} value, the only one in the entire time period does 
not exclude a single candidate that would not already be within the time window. 
\textit{ipHash} thus demonstrably contributes nothing to the separation in this dataset. 
The exact opposite of Section~\ref{subsubsec:influence-of-iphash}, where \texttt{IP\_ONLY} coincided 
with \texttt{HASH\_AND\_TIME\_WINDOW} because in that case, the IP address alone was sufficient for 
filtering.

The precision of \texttt{TIME\_WINDOW} at 73.67\%, the value falls between the result of 
Section~\ref{subsubsec:precision-time-window-multi-user} (65.00\%, synthetic HTTP harness, single 
process) and Section~\ref{subsubsec:precision-multiple-browser-users} (89.93\%, real browsers, 
separate containers). 
This aligns with the density. 
504 out of 600 logins (84\%) have intervals under 100 ms, which is close to the 86.8\% in 
Section~\ref{subsubsec:precision-time-window-multi-user}, and significantly higher than the 
22.8\% in Section~\ref{subsubsec:precision-multiple-browser-users}. 
Multiple threads running without wait times from a single process generate traffic density 
similar to that of the serial HTTP harness from Section~\ref{subsubsec:precision-time-window-multi-user} 
and Section~\ref{subsubsec:precision-multiple-browser-users}, not the timing of real 
browser interactions from Section~\ref{subsubsec:precision-browser-traffic} and 
Section~\ref{subsubsec:precision-multiple-browser-users}. 
The effect thus stems primarily from the login density, not from the shared IP itself, which is 
entirely in line with the core message already established in 
Section~\ref{subsubsec:precision-multiple-browser-users}.
Offset is calculated by (\textit{receivedAt} - \textit{event\_time}): min = -50 ms, p50 = -26 ms, 
p95 = -21 ms, max = 110 ms (n = 600) in the same order of magnitude as the other submeasurements.
